...
func (e Extractor) FileRequired(api filesystem.FileAPI) bool {
	...
	if filepath.Ext(normalizedPath) != ".gem" {
		return false
	}
...
func (e Extractor) extractPackages(path string, r io.Reader) ([]*extractor.Package, error) {
	tr := tar.NewReader(r)
	...
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("reading gem tar archive %s: %w", path, err)
		}
		cleanName := filepath.Clean(filepath.ToSlash(hdr.Name))
		if cleanName == "metadata.gz" || filepath.Base(cleanName) == "metadata.gz" {
			metaGzReader = tr
			break
		}
	}
	if metaGzReader == nil {
		return nil, fmt.Errorf("metadata.gz not found in gem archive %s", path)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("reading decompressed metadata from %s: %w", path, err)
	}
	if int64(len(content)) > limit {
		return nil, fmt.Errorf("decompressed metadata in %s exceeds size limit of %d bytes", path, limit)
	}

	var spec gemSpecification
	if err := yaml.Unmarshal(content, &spec); err != nil {
		return nil, fmt.Errorf("parsing gem specification yaml in %s: %w", path, err)
	}

	if spec.Name == "" || string(spec.Version) == "" {
		log.Debugf("gem specification in %s does not have a valid name and/or version", path)
		return nil, nil
	}
